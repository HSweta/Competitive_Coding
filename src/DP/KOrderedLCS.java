...
public class KOrderedLCS {
	...
	public static void main(String[] args) {
		...
		String s2 = "agcfd";
		int k = 1;
		System.out.println(KOrderedLCSRec(s1, s2, 0, 0, 1));
		...
		System.out.println(kOrderedLCSTD(s1, s2, 1, 0, 0, strg));
		System.out.println(KOrderLCSBU(s1, s2, k));
	}

	public static int KOrderedLCSRec(String s1, String s2, int vidx1, int vidx2, int k) {
		...
		if (s1.charAt(vidx1) == s2.charAt(vidx2)) {
			ans = KOrderedLCSRec(s1, s2, vidx1 + 1, vidx2 + 1, k) + 1;
		} else {
			int o1 = KOrderedLCSRec(s1, s2, vidx1 + 1, vidx2, k);
			int o2 = KOrderedLCSRec(s1, s2, vidx1, vidx2 + 1, k);
			int o3 = 0;
			if (k >= 1)
				o3 = KOrderedLCSRec(s1, s2, vidx1 + 1, vidx2 + 1, k - 1) + 1;
